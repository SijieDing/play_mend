...
sys.path.extend(['/app/libs/nab_detection_pkg' ,'/app/libs/aiops_unite_pkg'])
from aiops_nab_detection.nab_detection import start as start_nab_detection_test
from aiops_nab_detection.config import init as config_inif
from aiops_nab_detection.kafka_utils import remove_group_id, init as init_kafka
...
def get_event_loop():
    global eventloop
    if eventloop is None or eventloop.is_closed():
...
class FuncApp(Flask):
    ...
    def signal_handler(self, signalnum, frame):
        global running
        self.logger.info('Flask received signal {} ...'.format(
    ...
    def metric_data(self):
        # This function will wait for the response from checking system
        eventloop = get_event_loop()
        event_data = request.get_json()
        def create_and_set_future(event_data):
            future = eventloop.create_future()
            event_data["resp_future"] = future
            event_data["resp_future_loop"] = eventloop
            return future
        
        async def wait_for_future(future):
            return await future
        
        future = create_and_set_future(event_data)
        nab_detection_mimic(event_data)
        result = eventloop.run_until_complete(wait_for_future(future))
        if result:
            return json.dumps(result), 200
        else:
            return "[]", 200

    def remove_group_id(self, groupid):
        context = {}
        config_inif(context)
        init_kafka(context)
        if remove_group_id(context, groupid):
            return f'Remove {groupid} successfully', 200
        else:
            return f'Remove {groupid} failed', 200
    
    def get_mem_snapshot(self):
        global LAST_SNAP_SHOT
        context = {}
        config_inif(context)
        if context["global_config"].get("trace_mem"):
            snapshot = tracemalloc.take_snapshot()
            stats = snapshot.statistics('traceback')
            result = "[ Top 5 memory-consuming allocations ] <br>"
            for stat in stats[:10]:
                result += f"Size: {stat.size / 1024 / 1024:.2f} MiB, Count: {stat.count}, Traceback: <br>"
                result += "&nbspCall Stack: <br>"
                for line in stat.traceback.format():
                    result += f"&nbsp&nbsp&nbsp{line} <br>"
            
            if LAST_SNAP_SHOT:
                top_stats = snapshot.compare_to(LAST_SNAP_SHOT, 'lineno')
                result += '<br> <br> <br>The difference of memory allocation <br>'
                for stat in top_stats[:10]:
                    result += f"&nbsp{stat} <br>"
                    for line in stat.traceback.format():
                        result += f"&nbsp&nbsp&nbsp{line} <br>"

            LAST_SNAP_SHOT = snapshot
            result += f'Collect gc <br> {gc.collect()}' 
            return result, 200
        else:
            # Get current memory usage
            process1 = psutil.Process(os.getpid())
            rss = process1.memory_info().rss
            result = f'Current memory usage: {rss / 1024 / 1024:.2f} MB <br>'
            
            # Dump objects to memory.pickle
            with open("memory.pickle", "wb") as dump:
                ...
                for obj in gc.get_objects():
                    if hasattr(obj, "__class__"):
                        ...
                        objects.append({"id": obj_id, "class": obj_class, "size": size, "referents": referents})
                pickle.dump(objects, dump)
            result += f"Dumped objects to memory.pickle"
            return result, 200
...
def main():
    ...
    app.add_url_rule('/snapshot', 'snapshot', app.get_mem_snapshot, methods=['GET'])
    app.add_url_rule('/groupid/<groupid>', 'groupdid', app.remove_group_id, methods=['DELETE'])
    app.add_url_rule('/metric_data', 'metric_data', app.metric_data, methods=['POST'])
    app.add_url_rule('/nab_demo', 'nab_demo', app.nab_demo, methods=['GET'])
    #
